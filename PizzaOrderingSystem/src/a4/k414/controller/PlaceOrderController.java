...
import a4.k414.view.PlaceOrderScreen;
	...
	class OrderListner implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			if(e.getSource()==takeOrderScreen.getPizzaButton()){
				...
				if(takeOrderScreen.getOrderTypeField().getSelectedItem().equals("Phone")){
					if(takeOrderScreen.gethomeAddressField().getText().isEmpty() || takeOrderScreen.getcityField().getText().isEmpty() 
							|| takeOrderScreen.getzipField().getText().isEmpty()){
						JOptionPane.showMessageDialog(null,"For Phone Orders, please enter the delivery address fields(Home Address, city and zipcode)");
						flag++;
					}
				}
				if(flag == 0){
